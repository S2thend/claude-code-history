...
As a developer consuming session history outside the interactive viewer, I want every message, tool input, and tool result returned in full so that exports, analysis, and downstream tooling never lose data because of presentation shortcuts.
...
**Independent Test**: Load a session containing long user messages, long assistant messages, long file-edit inputs, and long tool outputs through the session retrieval interface and verify that every field is returned exactly as recorded with no omission markers or length cuts.
...
**Independent Test**: Open a session with long tool inputs/results in full-detail mode and verify that all content appears with no omission markers and no shortened fields.
...
3. **Given** full-detail mode is enabled, **When** the viewer renders long text messages and tool content, **Then** no display-layer truncation or omission markers are applied to any message field.
...
**Independent Test**: Open a session with long tool inputs/results in default viewing mode and verify that the output stays readable, any abbreviations are clearly indicated, and rerunning the same view in full-detail mode reveals the complete content.

**Acceptance Scenarios**:

1. **Given** a session contains very long tool inputs or tool results, **When** the user opens the session in default viewing mode, **Then** the viewer may abbreviate those long fields for readability.
2. **Given** a field is abbreviated in default viewing mode, **When** the viewer renders that field, **Then** the user can clearly tell that content was omitted from display rather than missing from the underlying data.
...
- What happens when source content already contains text that looks like an omission marker? Programmatic retrieval preserves the source text exactly, and default viewing must not make source-authored text indistinguishable from display-added abbreviations.
...
- **FR-001**: Session retrieval for programmatic consumers MUST return the full, untruncated content of every user message, assistant message, tool input, and tool result field.
...
- **FR-004**: Default command-line session viewing MAY abbreviate long tool inputs and long tool results for readability, but any abbreviation MUST be applied only while rendering display output.
- **FR-005**: Any abbreviation shown in default command-line session viewing MUST clearly indicate that the field was shortened for display.
- **FR-006**: Full-detail command-line viewing MUST display the complete content of all message text, tool inputs, and tool results with no field-level abbreviation.
...
- Default command-line viewing may keep concise abbreviations for long tool fields as long as those abbreviations are display-only and clearly marked.
...
- **SC-003**: In acceptance testing with long tool payloads, 100% of default session-view checks that abbreviate content also show a clear user-visible indication that the field was shortened for display.
